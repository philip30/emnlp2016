...
for (( tc=0; tc < $EPOCH; tc++ )) do
    if [ -e ${model_out}-$tc ]; then
        ...
        python3 $CHAINN/nmt.py --init_model ${model_out}-$tc $gpu --verbose $decoder_options < ${data}/test.en --align_out ${test_name}-single.align > ${test_name}-single.out 2> ${test_name}-single.log
